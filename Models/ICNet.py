...
def ICNet(nClasses, input_height=224, input_width=224):
    ...
    y = Lambda(lambda x: tf.image.resize_bilinear(
        x, size=(int(x.shape[1]) // 2, int(x.shape[2]) // 2)),
    ...
    y_ = Conv2D(64, 1, strides=2, activation='relu',
                name='conv3_1_1x1_reduce')(y_)
    ...
    y_ = Conv2D(128, 3, dilation_rate=2, activation='relu',
                name='conv4_1_3x3')(y_)
    ...
    y = Conv2D(128, 3, dilation_rate=2, activation='relu',
               name='conv4_2_3x3')(y)
    ...
    y = Conv2D(256, 3, dilation_rate=4, activation='relu',
               name='conv5_1_3x3')(y)
    ...
    pool1 = AveragePooling2D(pool_size=(h, w),
                             strides=(h, w),
                             name='conv5_3_pool1')(y)
    pool1 = Lambda(lambda x: tf.image.resize_bilinear(x, size=(h, w)),
                   name='conv5_3_pool1_interp')(pool1)
    pool2 = AveragePooling2D(pool_size=(h / 2, w / 2),
                             strides=(h // 2, w // 2),
                             name='conv5_3_pool2')(y)
    pool2 = Lambda(lambda x: tf.image.resize_bilinear(x, size=(h, w)),
                   name='conv5_3_pool2_interp')(pool2)
    pool3 = AveragePooling2D(pool_size=(h / 3, w / 3),
                             strides=(h // 3, w // 3),
                             name='conv5_3_pool3')(y)
    pool3 = Lambda(lambda x: tf.image.resize_bilinear(x, size=(h, w)),
                   name='conv5_3_pool3_interp')(pool3)
    pool6 = AveragePooling2D(pool_size=(h / 4, w / 4),
                             strides=(h // 4, w // 4),
                             name='conv5_3_pool6')(y)
    pool6 = Lambda(lambda x: tf.image.resize_bilinear(x, size=(h, w)),
